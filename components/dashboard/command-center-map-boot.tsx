...
export const COMMAND_CENTER_MAP_CANVAS_BG = "#09090b";

type CommandCenterMapBootOverlayProps = {
  visible: boolean;
  icon: LucideIcon;
  title?: string;
  subtitle?: string;
...
/**
 * Повний чорний прелоадер поверх карти (включно під списком).
 * Текст центрується у видимій смузі праворуч від glass-панелі.
 */
export function CommandCenterMapBootOverlay({
  visible,
  icon: Icon,
  title = "Підготовка карти",
  subtitle,
}: CommandCenterMapBootOverlayProps) {
  return (
    <div
      className={cn(
        "absolute inset-0 z-50 overflow-hidden bg-zinc-950 transition-opacity duration-700 ease-out",
