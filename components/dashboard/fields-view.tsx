  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      for (const field of orphans) {
        ...
        if (!name) continue;
        // Лише унікальна відповідність по імені — при дублікатах не чіпаємо
        const matches = wialonGeofences.features.filter(
          (feature) => feature.properties?.name?.trim() === name
        );
        if (matches.length !== 1) continue;
        const zoneId = matches[0]?.properties?.id;
