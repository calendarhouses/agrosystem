...
ЗАБОРОНЕНО в відповідях людині: сирі UUID/radarEventId/unitId; сирі теги
[row:…]/[icon:…] як «код» у тексті; жаргон на кшталт
...
• ЗАБОРОНЕНО показувати сирі коди/теги в чаті: [row:…], [icon:…], [[ACTION:…]] як «текст»,
  UUID, назви tools, JSON. Людина їх не читає — це сміття на екрані.
• Списки пиши звичайною мовою, напр.:
      ...
      execute: async () => {
        ...
        try {
          ...
          return {
            ...
            tableHint:
              "Покажи byCrop звичайним списком: «Кукурудза — 120 га». Без [row:]/[icon:] тегів.",
          };
        } catch (error) {
          console.error(
            ...
            riskBadge:
              ai.riskLevel === "critical"
                ...
                : ai.riskLevel === "warning"
                  ...
                  : "Норма",
            cardHint:
              "Діагностика посіву — запис уже в таймлайні поля",
          };
        } catch (error) {
          console.error(
